...
ComportThread::ComportThread()
    : fd_(-1), run_flag_(false), file_pathname_("") {}

ComportThread::ComportThread(const SETTING &setting)
    : fd_(-1), run_flag_(false), setting_(setting), file_pathname_("") {}

ComportThread::~ComportThread() { Disconnect(); }

void ComportThread::PortSetting(const SETTING &setting) { setting_ = setting; }
